...
import {
  ...
  BookOpen, Brain, Sparkles, FileSearch, X, File,
} from "lucide-react"
...
export default function DashboardPage() {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-[1200px] mx-auto space-y-6">
      {/* ═══════ Welcome Header + Upload ═══════ */}
      <motion.section variants={itemVariants} className="bg-gradient-to-l from-white via-white to-[#f0faf4] dark:from-[#1e293b] dark:via-[#1e293b] dark:to-[#0f172a] rounded-2xl p-8 shadow-sm border border-[#e8ecf1] dark:border-slate-800 relative overflow-hidden transition-colors duration-300">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row gap-8 items-center">
            ...
            <AnimatePresence mode="wait">
              {uploadState === "idle" ? (
                <motion.div key="upload-idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  className={`flex flex-col items-center justify-center border-2 border-dashed rounded-2xl py-8 px-6 cursor-pointer transition-all duration-300
                    ${isDragOver 
                      ...
                      : "border-[#d1d5db] dark:border-slate-700 hover:border-[#006C35]/40 dark:hover:border-[#34d399]/40 hover:bg-[#f0faf4] dark:hover:bg-slate-800 hover:shadow-md"}`}
                >
                  <motion.div animate={isDragOver ? { scale: 1.2, rotate: 5 } : { scale: 1, rotate: 0 }}
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-3 transition-colors ${isDragOver ? "bg-[#006C35]/10 dark:bg-[#006C35]/30" : "bg-[#f0faf4] dark:bg-slate-800"}`}>
                    <Cloud className={`w-7 h-7 transition-colors ${isDragOver ? "text-[#006C35] dark:text-[#34d399]" : "text-[#00A651] dark:text-[#4ade80]"}`} />
                  </motion.div>
                  <p className="text-[15px] font-semibold text-[#1a202c] dark:text-slate-200 mb-1">رفع ملف PDF هنا</p>
                  <p className="text-[12px] text-[#9ca3af] dark:text-slate-400 mb-4">أو اسحب وألقِ الملف</p>
                  ...
                  <p className="text-[10px] text-[#d1d5db] dark:text-slate-500 mt-3">الحد الأقصى لحجم الملف 50 ميجابايت</p>
                </motion.div>
              ) : uploadState === "uploading" ? (
                <motion.div key="upload-progress" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                  className="border-2 border-[#006C35]/20 dark:border-[#34d399]/20 bg-[#f0faf4] dark:bg-slate-800 rounded-2xl py-8 px-6 text-center">
                  ...
                  <p className="text-[14px] font-semibold text-[#1a202c] dark:text-slate-200 mb-1 truncate">{uploadedFile?.name}</p>
                  <p className="text-[12px] text-[#9ca3af] dark:text-slate-400 mb-4">{uploadedFile?.size}</p>
                  <div className="w-full h-2.5 bg-[#d1e7dd] dark:bg-slate-700 rounded-full overflow-hidden mb-2">
                ...
                <motion.div key="upload-success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                  className="border-2 border-[#006C35]/20 dark:border-[#34d399]/20 bg-[#f0faf4] dark:bg-slate-800 rounded-2xl py-8 px-6 text-center relative">
                  <button onClick={resetUpload} className="absolute top-3 left-3 w-6 h-6 rounded-full bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors">
                    <X className="w-3.5 h-3.5 text-[#9ca3af] dark:text-slate-300" />
                  </button>
                  ...
                  <p className="text-[12px] text-[#6b7280] dark:text-slate-400 mb-4">{uploadedFile?.name}</p>
                  <div className="flex gap-2 justify-center">
                    ...
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={resetUpload}
                      className="px-4 py-2 bg-white dark:bg-slate-700 border border-[#e8ecf1] dark:border-slate-600 text-[#6b7280] dark:text-slate-300 text-[12px] font-semibold rounded-xl hover:bg-[#f4f6f9] dark:hover:bg-slate-600 transition-colors">
                      رفع ملف آخر
                    </motion.button>
                  </div>
                </motion.div>
          ...
          <div className="flex-1 text-center lg:text-right order-1 lg:order-1">
            ...
            <p className="text-[15px] text-[#6b7280] dark:text-slate-400 leading-relaxed max-w-lg lg:mr-0 mx-auto">
            ...
            <div className="flex gap-3 mt-5 justify-center lg:justify-start">
              ...
              <Link href="/dashboard/documents">
                <motion.button whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}
                  className="px-5 py-2.5 bg-white dark:bg-slate-800 border border-[#e8ecf1] dark:border-slate-700 text-[#374151] dark:text-slate-300 text-[13px] font-semibold rounded-xl hover:bg-[#f4f6f9] dark:hover:bg-slate-700 transition-all">
                  عرض المستندات
                </motion.button>
              </Link>
            </div>
      ...
      <motion.section variants={itemVariants}>
        <h2 className="text-[16px] font-bold text-[#1a202c] dark:text-slate-200 mb-3 flex items-center gap-2">
          ...
          {smartActions.map((action, i) => (
            <Link key={action.label} href={action.href}>
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }}
                ...
                <h3 className="text-[13px] font-bold text-[#1a202c] dark:text-slate-200 mb-0.5">{action.label}</h3>
                <p className="text-[11px] text-[#6b7280] dark:text-slate-400 leading-relaxed">{action.description}</p>
              </motion.div>
            </Link>
          ))}
        </div>
        ...
        {quickActions.map((action, i) => (
          <Link key={action.label} href={action.href}>
            <motion.div whileHover={{ y: -3, scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className="group bg-white dark:bg-[#1e293b] rounded-2xl p-5 border border-[#e8ecf1] dark:border-slate-800 hover:shadow-lg hover:shadow-[#006C35]/5 dark:hover:shadow-black/50 hover:border-[#006C35]/20 dark:hover:border-slate-700 transition-all duration-300 text-center cursor-pointer">
              <div className="w-12 h-12 rounded-xl mx-auto mb-3 flex items-center justify-center bg-[#f0faf4] dark:bg-slate-800 transition-transform group-hover:scale-110 duration-300">
              ...
              <h3 className="text-[14px] font-bold text-[#1a202c] dark:text-slate-200 mb-1">{action.label}</h3>
              <p className="text-[11px] text-[#9ca3af] dark:text-slate-400 leading-relaxed">{action.description}</p>
            </motion.div>
          </Link>
        ))}
      </motion.section>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* Recent Documents */}
        <section className="lg:col-span-3 bg-white dark:bg-[#1e293b] rounded-2xl border border-[#e8ecf1] dark:border-slate-800 overflow-hidden hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8ecf1] dark:border-slate-800">
            <h2 className="text-[16px] font-bold text-[#1a202c] dark:text-slate-200">المستندات الأخيرة</h2>
            ...
            {recentDocuments.map((doc, index) => (
              <motion.div key={index} whileHover={{ backgroundColor: "var(--hover-bg)" }}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-semibold text-[#1a202c] dark:text-slate-200 truncate group-hover:text-[#006C35] dark:group-hover:text-[#34d399] transition-colors">{doc.name}</p>
                  <div className="flex items-center gap-2">
                    <p className="text-[12px] text-[#9ca3af] dark:text-slate-400">{doc.pages} صفحة</p>
                    {doc.status === "analyzed" && (
                ...
                <div className="text-left shrink-0">
                  <p className="text-[12px] text-[#6b7280] dark:text-slate-400">{doc.date}</p>
                  <p className="text-[11px] text-[#d1d5db] dark:text-slate-500">{doc.time}</p>
                </div>
                <button className="opacity-0 group-hover:opacity-100 transition-opacity w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#f0f2f5] dark:hover:bg-slate-700">
                  <MoreVertical className="w-4 h-4 text-[#9ca3af] dark:text-slate-400" />
                </button>
              </motion.div>
            ))}
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <section className="bg-white dark:bg-[#1e293b] rounded-2xl border border-[#e8ecf1] dark:border-slate-800 p-6 hover:shadow-md transition-shadow">
            <h2 className="text-[16px] font-bold text-[#1a202c] dark:text-slate-200 mb-5">ملخص استخدامك</h2>
            <div className="grid grid-cols-2 gap-4">
              {usageStats.map((stat, i) => (
                <motion.div key={stat.label} whileHover={{ y: -2, scale: 1.03 }}
                  className="text-center p-4 rounded-xl bg-[#f8f9fb] dark:bg-slate-800 border border-[#e8ecf1] dark:border-slate-700 hover:shadow-sm transition-all cursor-default">
                  ...
                  <p className="text-[13px] font-semibold text-[#374151] dark:text-slate-300">{stat.label}</p>
                  <p className="text-[10px] text-[#9ca3af] dark:text-slate-500">{stat.sublabel}</p>
                </motion.div>
              ))}
            </div>
          </section>
          ...
          <motion.section whileHover={{ scale: 1.01 }}
            ...
            <div className="flex items-start gap-4 relative z-10">
              ...
              <div className="w-16 h-16 bg-white/60 dark:bg-slate-800/60 rounded-2xl flex items-center justify-center shrink-0 shadow-sm border border-white/50 dark:border-white/5">
